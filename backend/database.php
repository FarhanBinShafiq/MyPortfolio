...
try {
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS blogs (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, date TEXT, category TEXT, excerpt TEXT, content TEXT, external_link TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)");
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS skills (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, category TEXT)");

    // SEED DEFAULT HOME DATA (Crucial for index.html to work)
    $checkHome = $pdo->query("SELECT COUNT(*) FROM home WHERE id=1")->fetchColumn();
    if ($checkHome == 0) {
        $pdo->exec("INSERT INTO home (id, volume_label, main_title, subtitle, role_text, avatar_url) 
                   VALUES (1, 'VOL. 1', 'Your Name', 'Portfolio', 'Full Stack Developer', '')");
    }

    // Seed Default Profile
    $checkProfile = $pdo->query("SELECT COUNT(*) FROM profile WHERE id=1")->fetchColumn();
    if ($checkProfile == 0) {
        $pdo->exec("INSERT INTO profile (id, intro_title, intro_body, bio_text, residence, experience, email, image_url) 
                   VALUES (1, 'H', 'ello.', 'Bio text...', 'USA', '5 Years', 'alex@dev.com', '')");
    }
